...
local rconsoleDataStore = game:GetService("DataStoreService"):GetDataStore("DTC_RConsole")
local workspaceDataStore = game:GetService("DataStoreService"):GetDataStore("DTC_Workspace")
local scriptCache = {}
...
local function getScriptKey(scriptName)
    return "script_" .. tostring(scriptName):gsub("[^%w_]", "_")
end

local function getAutoExecuteKey(scriptName)
    return "autoexe_" .. tostring(scriptName):gsub("[^%w_]", "_")
end

local function getRConsoleKey(entryName)
    return "rconsole_" .. tostring(entryName):gsub("[^%w_]", "_")
end

local function getWorkspaceKey(fileName)
    return "workspace_" .. tostring(fileName):gsub("[^%w_]", "_")
end

local function acquireLock(key)
    while registryLock[key] do
        wait(0.1)
    end
    registryLock[key] = true
    return true
end

local function releaseLock(key)
    registryLock[key] = nil
end

local function queueOperation(opType, ...)
    local opId = #operationQueue + 1
    operationQueue[opId] = {type = opType, args = {...}, time = os.time()}
    if #operationQueue > 100 then
        table.remove(operationQueue, 1)
    end
    return opId
end
...
local function validateScriptName(name)
    if type(name) ~= "string" then return false, "Name must be string" end
    if #name == 0 then return false, "Name cannot be empty" end
    if #name > 100 then return false, "Name too long" end
    if not name:match("^[%w%s%._-]+$") then return false, "Invalid characters" end
    return true
end

local function validateScriptContent(content)
    if type(content) ~= "string" then return false, "Content must be string" end
    if #content > 1000000 then return false, "Content too large" end
    return true
end

local function safeDatastoreCall(datastore, method, ...)
    local maxRetries = 3
    local retryDelay = 1
    
    for attempt = 1, maxRetries do
        local success, result = pcall(function()
            return datastore[method](datastore, ...)
        end)
...
local function updateScriptRegistry(scriptName, action)
    if not acquireLock("script_registry") then return false end
    
    local success, result = pcall(function()
        local registrySuccess, scriptList = safeDatastoreCall(scriptDataStore, "GetAsync", "_script_registry")
        ...
        elseif action == "remove" then
            for i = #scriptList, 1, -1 do
                if scriptList[i] == scriptName then
        ...
        elseif action == "clean" then
            ...
            for _, name in ipairs(scriptList) do
                local contentSuccess = safeDatastoreCall(scriptDataStore, "GetAsync", getScriptKey(name))
        ...
        if modified then
            local saveSuccess = safeDatastoreCall(scriptDataStore, "SetAsync", "_script_registry", scriptList)
    ...
    releaseLock("script_registry")
    return success, result
end

local function updateAutoExecuteRegistry(scriptName, action)
    if not acquireLock("autoexe_registry") then return false end
    
    local success, result = pcall(function()
        local registrySuccess, autoExeList = safeDatastoreCall(autoExecuteDataStore, "GetAsync", "_autoexe_registry")
        
        if not registrySuccess then
            autoExeList = {}
        else
            autoExeList = autoExeList or {}
        end
        ...
        if action == "add" then
            local exists = false
            for i, v in ipairs(autoExeList) do
                if v == scriptName then
                    exists = true
                    break
                end
        ...
        elseif action == "remove" then
            for i = #autoExeList, 1, -1 do
                if autoExeList[i] == scriptName then
                    table.remove(autoExeList, i)
                    modified = true
                    break
                end
            end
        elseif action == "clean" then
            local newList = {}
            for _, name in ipairs(autoExeList) do
                local contentSuccess = safeDatastoreCall(autoExecuteDataStore, "GetAsync", getAutoExecuteKey(name))
                if contentSuccess then
                    table.insert(newList, name)
                end
            end
        ...
        if modified then
            local saveSuccess = safeDatastoreCall(autoExecuteDataStore, "SetAsync", "_autoexe_registry", autoExeList)
            if not saveSuccess then
                return false, "Failed to save registry"
            end
        end
        
        return true, autoExeList
    end)
    
    releaseLock("autoexe_registry")
    return success, result
end

local function updateRConsoleRegistry(entryName, action)
    if not acquireLock("rconsole_registry") then return false end
    
    local success, result = pcall(function()
        local registrySuccess, rconsoleList = safeDatastoreCall(rconsoleDataStore, "GetAsync", "_rconsole_registry")
        
        if not registrySuccess then
            rconsoleList = {}
        else
            rconsoleList = rconsoleList or {}
        end
        ...
        if action == "add" then
            local exists = false
            for i, v in ipairs(rconsoleList) do
                if v == entryName then
                    exists = true
                    break
                end
            ...
            if not exists then
                table.insert(rconsoleList, entryName)
                modified = true
            end
        elseif action == "remove" then
            for i = #rconsoleList, 1, -1 do
                if rconsoleList[i] == entryName then
                    table.remove(rconsoleList, i)
                    modified = true
                    break
                end
            end
        elseif action == "clean" then
            local newList = {}
            for _, name in ipairs(rconsoleList) do
                local contentSuccess = safeDatastoreCall(rconsoleDataStore, "GetAsync", getRConsoleKey(name))
                if contentSuccess then
                    table.insert(newList, name)
                end
            end
            rconsoleList = newList
            modified = true
        end

        if modified then
            local saveSuccess = safeDatastoreCall(rconsoleDataStore, "SetAsync", "_rconsole_registry", rconsoleList)
            if not saveSuccess then
                return false, "Failed to save registry"
            end
        end
        
        return true, rconsoleList
    end)
    
    releaseLock("rconsole_registry")
    return success, result
end

local function updateWorkspaceRegistry(fileName, action)
    if not acquireLock("workspace_registry") then return false end
    
    local success, result = pcall(function()
        local registrySuccess, workspaceList = safeDatastoreCall(workspaceDataStore, "GetAsync", "_workspace_registry")
        
        if not registrySuccess then
            workspaceList = {}
        else
            workspaceList = workspaceList or {}
        end
        ...
        if action == "add" then
            ...
            if not exists then
                table.insert(workspaceList, fileName)
                modified = true
            end
        elseif action == "remove" then
            for i = #workspaceList, 1, -1 do
                if workspaceList[i] == fileName then
                    table.remove(workspaceList, i)
                    modified = true
                    break
                end
            end
        elseif action == "clean" then
            local newList = {}
            for _, name in ipairs(workspaceList) do
                local contentSuccess = safeDatastoreCall(workspaceDataStore, "GetAsync", getWorkspaceKey(name))
                if contentSuccess then
                    table.insert(newList, name)
                end
            end
        ...
        if modified then
            local saveSuccess = safeDatastoreCall(workspaceDataStore, "SetAsync", "_workspace_registry", workspaceList)
            if not saveSuccess then
                return false, "Failed to save registry"
            end
        end
    ...
    releaseLock("workspace_registry")
    return success, result
end
...
local function initializeRegistries()
    pcall(function()
        ...
        if not currentScripts then
            ...
            updateScriptRegistry(nil, "clean")
        end
        ...
        if not currentAutoExe then
            ...
            updateAutoExecuteRegistry(nil, "clean")
        end

        local currentRConsole = rconsoleDataStore:GetAsync("_rconsole_registry")
        if not currentRConsole then
            rconsoleDataStore:SetAsync("_rconsole_registry", {})
        else
            updateRConsoleRegistry(nil, "clean")
        end

        local currentWorkspace = workspaceDataStore:GetAsync("_workspace_registry")
        if not currentWorkspace then
            workspaceDataStore:SetAsync("_workspace_registry", {})
        else
            updateWorkspaceRegistry(nil, "clean")
        end
    end)
end

-- RCONSOLE FUNCTIONS
function dtc.writerconsole(entryName, content)
    local validName, nameErr = validateScriptName(entryName)
    if not validName then
        warn("Invalid rconsole entry name: " .. tostring(nameErr))
        return false
    end
    
    local validContent, contentErr = validateScriptContent(content)
    if not validContent then
        warn("Invalid rconsole content: " .. tostring(contentErr))
        return false
    end
    
    queueOperation("writerconsole", entryName, #content)

    local success, errorMsg = safeDatastoreCall(rconsoleDataStore, "SetAsync", getRConsoleKey(entryName), content)

    if success then
        rconsoleCache[entryName] = content
        updateRConsoleRegistry(entryName, "add")
        return true
    else
        warn("Failed to write rconsole entry '" .. entryName .. "': " .. tostring(errorMsg))
        return false
    end
end

function dtc.readrconsole(entryName)
    local valid, err = validateScriptName(entryName)
    if not valid then
        warn("Invalid rconsole entry name: " .. tostring(err))
        return nil
    end
    
    queueOperation("readrconsole", entryName)
    
    if rconsoleCache[entryName] then
        return rconsoleCache[entryName]
    end

    local success, content = safeDatastoreCall(rconsoleDataStore, "GetAsync", getRConsoleKey(entryName))
...
function dtc.delfilerconsole(entryName)
    local valid, err = validateScriptName(entryName)
    if not valid then
        warn("Invalid rconsole entry name: " .. tostring(err))
        return false
    ...
    queueOperation("delfilerconsole", entryName)

    local success = safeDatastoreCall(rconsoleDataStore, "RemoveAsync", getRConsoleKey(entryName))

    rconsoleCache[entryName] = nil
    updateRConsoleRegistry(entryName, "remove")
    return success
end

function dtc.isfilerconsole(entryName)
    local valid, err = validateScriptName(entryName)
    if not valid then
        return false
    end
    
    queueOperation("isfilerconsole", entryName)
    
    if rconsoleCache[entryName] then
        return true
    end

    local content = dtc.readrconsole(entryName)
    return content ~= nil and type(content) == "string" and #content > 0
end

function dtc.listrconsole()
    queueOperation("listrconsole")
    
    local registrySuccess, rconsoleList = safeDatastoreCall(rconsoleDataStore, "GetAsync", "_rconsole_registry")
    
    if registrySuccess and type(rconsoleList) == "table" then
        local validEntries = {}
        for _, name in ipairs(rconsoleList) do
            if dtc.isfilerconsole(name) then
                table.insert(validEntries, name)
            end
        end
        return validEntries
    end
...
-- WORKSPACE FUNCTIONS
function dtc.writeworkspace(fileName, content)
    local validName, nameErr = validateScriptName(fileName)
    if not validName then
        warn("Invalid workspace file name: " .. tostring(nameErr))
        return false
    end
    
    local validContent, contentErr = validateScriptContent(content)
    if not validContent then
        warn("Invalid workspace content: " .. tostring(contentErr))
        return false
    end
    
    queueOperation("writeworkspace", fileName, #content)

    local success, errorMsg = safeDatastoreCall(workspaceDataStore, "SetAsync", getWorkspaceKey(fileName), content)

    if success then
        workspaceCache[fileName] = content
        updateWorkspaceRegistry(fileName, "add")
        return true
    else
        warn("Failed to write workspace file '" .. fileName .. "': " .. tostring(errorMsg))
        return false
    end
end

function dtc.readworkspace(fileName)
    local valid, err = validateScriptName(fileName)
    if not valid then
        warn("Invalid workspace file name: " .. tostring(err))
        return nil
    end
    
    queueOperation("readworkspace", fileName)
    
    if workspaceCache[fileName] then
        return workspaceCache[fileName]
    end

    local success, content = safeDatastoreCall(workspaceDataStore, "GetAsync", getWorkspaceKey(fileName))
...
function dtc.delfileworkspace(fileName)
    local valid, err = validateScriptName(fileName)
    if not valid then
        warn("Invalid workspace file name: " .. tostring(err))
        return false
    ...
    queueOperation("delfileworkspace", fileName)

    local success = safeDatastoreCall(workspaceDataStore, "RemoveAsync", getWorkspaceKey(fileName))

    workspaceCache[fileName] = nil
    updateWorkspaceRegistry(fileName, "remove")
    return success
end

function dtc.isfileworkspace(fileName)
    local valid, err = validateScriptName(fileName)
    if not valid then
        return false
    end
    
    queueOperation("isfileworkspace", fileName)
    
    if workspaceCache[fileName] then
        return true
    end

    local content = dtc.readworkspace(fileName)
    return content ~= nil and type(content) == "string" and #content > 0
end

function dtc.listworkspace()
    queueOperation("listworkspace")
    
    local registrySuccess, workspaceList = safeDatastoreCall(workspaceDataStore, "GetAsync", "_workspace_registry")
    
    if registrySuccess and type(workspaceList) == "table" then
        local validFiles = {}
        for _, name in ipairs(workspaceList) do
            if dtc.isfileworkspace(name) then
...
function dtc.schedule(callback, delay, ...)
    ...
    local args = {...}
    queueOperation("schedule", delay, #args)
    
    task.delay(delay, function()
        local success, err = pcall(callback, unpack(args))
        if not success then
            warn("Scheduled task failed: " .. tostring(err))
        end
    end)
end

function dtc.pushautoexec()
    queueOperation("pushautoexec")
    ...
    for _, scriptName in ipairs(autoExeScripts) do
        ...
        if scriptContent and type(scriptContent) == "string" and #scriptContent > 0 then
            dtc.schedule(function()
                ...
                if not success then
                    warn("Auto-execute failed for " .. tostring(scriptName) .. ": " .. tostring(error))
                end
            end, 1)
        end
    end
...
function dtc.insane(value)
    if value ~= nil then
        if type(value) == "number" and value >= 0 then
            getgenv()._INSANE_COUNT = math.floor(value)
            return true
        else
            return false
        end
    end
    return tonumber(getgenv()._INSANE_COUNT) or 0
end

function dtc.readscript(scriptName)
    local valid, err = validateScriptName(scriptName)
    if not valid then
        warn("Invalid script name: " .. tostring(err))
        return nil
    end
    
    queueOperation("readscript", scriptName)
    
    if scriptCache[scriptName] then
        return scriptCache[scriptName]
    end

    local success, content = safeDatastoreCall(scriptDataStore, "GetAsync", getScriptKey(scriptName))
...
function dtc.writescript(scriptName, content)
    local validName, nameErr = validateScriptName(scriptName)
    if not validName then
        warn("Invalid script name: " .. tostring(nameErr))
        return false
    end
    
    local validContent, contentErr = validateScriptContent(content)
    if not validContent then
        warn("Invalid script content: " .. tostring(contentErr))
        return false
    end
    
    queueOperation("writescript", scriptName, #content)

    local success, errorMsg = safeDatastoreCall(scriptDataStore, "SetAsync", getScriptKey(scriptName), content)
    ...
    if success then
        ...
        warn("Failed to write script '" .. scriptName .. "': " .. tostring(errorMsg))
        return false
    end
end
...
function dtc.isfilescript(scriptName)
    ...
    queueOperation("isfilescript", scriptName)
...
function dtc.delfilescript(scriptName)
    ...
    queueOperation("delfilescript", scriptName)

    local success = safeDatastoreCall(scriptDataStore, "RemoveAsync", getScriptKey(scriptName))
...
function dtc.listscripts()
    queueOperation("listscripts")
    
    local registrySuccess, scriptList = safeDatastoreCall(scriptDataStore, "GetAsync", "_script_registry")
    
    if registrySuccess and type(scriptList) == "table" then
        local validScripts = {}
        for _, name in ipairs(scriptList) do
            if dtc.isfilescript(name) then
                table.insert(validScripts, name)
            end
        end
        return validScripts
    end
...
function dtc.readautoexe(scriptName)
    ...
    queueOperation("readautoexe", scriptName)
    
    if autoExecuteCache[scriptName] then
        return autoExecuteCache[scriptName]
    end

    local success, content = safeDatastoreCall(autoExecuteDataStore, "GetAsync", getAutoExecuteKey(scriptName))
...
function dtc.create_autoexe(scriptName, content)
    ...
    queueOperation("create_autoexe", scriptName, #content)

    local success, errorMsg = safeDatastoreCall(autoExecuteDataStore, "SetAsync", getAutoExecuteKey(scriptName), content)
    ...
    if success then
        ...
        warn("Failed to create auto-execute '" .. scriptName .. "': " .. tostring(errorMsg))
        return false
    end
end
...
function dtc.isfileautoexe(scriptName)
    ...
    queueOperation("isfileautoexe", scriptName)
...
function dtc.delfileautoexe(scriptName)
    ...
    queueOperation("delfileautoexe", scriptName)

    local success = safeDatastoreCall(autoExecuteDataStore, "RemoveAsync", getAutoExecuteKey(scriptName))
...
function dtc.listautoexe()
    queueOperation("listautoexe")
    
    local registrySuccess, autoExeList = safeDatastoreCall(autoExecuteDataStore, "GetAsync", "_autoexe_registry")
    
    if registrySuccess and type(autoExeList) == "table" then
        local validScripts = {}
        for _, name in ipairs(autoExeList) do
            if dtc.isfileautoexe(name) then
                table.insert(validScripts, name)
            end
        end
        return validScripts
...
function dtc.getoperationlog()
    return deepCopy(operationQueue)
end

function dtc.clearcache()
    scriptCache = {}
    autoExecuteCache = {}
    rconsoleCache = {}
    workspaceCache = {}
...
function dtc.getstats()
    local scripts = dtc.listscripts()
    local autoexe = dtc.listautoexe()
    local rconsole = dtc.listrconsole()
    local workspace = dtc.listworkspace()
    
    return {
        scriptCount = #scripts,
        autoExeCount = #autoexe,
        rconsoleCount = #rconsole,
        workspaceCount = #workspace,
        cacheSize = {
            scripts = #scriptCache,
            autoexe = #autoExecuteCache,
            rconsole = #rconsoleCache,
            workspace = #workspaceCache
        },
        operationCount = #operationQueue,
        insaneCount = dtc.insane()
    }
end
...
if not script:IsA("ModuleScript") then
    ...
    do
        ...
        copy_func("writerconsole")
        copy_func("readrconsole")
        copy_func("delfilerconsole")
        copy_func("isfilerconsole")
        copy_func("listrconsole")
        copy_func("writeworkspace")
        copy_func("readworkspace")
        copy_func("delfileworkspace")
        copy_func("isfileworkspace")
        copy_func("listworkspace")
        copy_func("getoperationlog")
        copy_func("clearcache")
        copy_func("getstats")
