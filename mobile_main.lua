...
UI["13"].Activated:Connect(function()
    -- When the user clicks the "CHECK KEY" button we immediately start a verification
    -- process for the provided key. Running the check in a spawned task prevents the UI
    -- thread from hanging if the HTTP request takes a while or errors out.
    local key = UI["9"].Text
    if key == "" then
    ...
    -- Set the status to checking so the user knows something is happening.
    updateStatus("Status: Checking...", Color3.fromRGB(255, 255, 255))

    -- Verify the key asynchronously to avoid blocking the main thread.
    async.on(function()
        -- Wrap the call in pcall so any thrown error doesn't break the UI.
        local ok, result = pcall(iskeygucci, key)
        local gucci = ok and result

        -- If the key check failed unexpectedly (e.g. a network error), record an error code
        -- so we can show the proper message.
        if not ok then
            error_key_code = "NETWORK_ERROR"
        end
        ...
        -- For invalid or failed keys we update the status and provide an appropriate notification.
        if error_key_code == "NETWORK_ERROR" then
            updateStatus("Status: Failed to check key", Color3.fromRGB(255, 100, 100))
            notify("An error occurred while verifying the key. Please check your connection and try again later.")
        else
