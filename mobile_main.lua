...
if is_beta() then repeat task.wait() until betaapi end
...
local KeySystemContainer = cloneref( gethui() );
...
UI["13"].Activated:Connect(function()
    if not api then
        notify("System is still loading, please wait...")
        return
    end
    ...
    task.spawn(function()
        ...
        if (gucci) then
            ...
            if currentBlur then
                ...
                task.delay(0.5, function()
                    if currentBlur then currentBlur:Destroy() end
                end)
            end

            if UI["1"] then UI["1"]:Destroy() end
            return;
        end
        ...
        elseif error_key_code == "KEY_INVALID" then
            notify("Key format is invalid (check spaces)");
        elseif error_key_code == "KEY_BANNED" then
             notify("Key is blacklisted");
        else
            notify("Key is Invalid or Incorrect :c");
        end
    end)
end)
