...
local function pcall_timeout_progress(timeout_seconds, on_tick, fn)
    ...
    local deadline = start_t + (timeout_seconds or 10)
    local last_tick = -1
    while not done and os.clock() < deadline do
        local elapsed = math.floor(os.clock() - start_t)
...
local function save_key(key)
    ...
    rconsoleprint("load_script: begin (stable)")

    local ok_call = false
    local err_call
    if type(api.load_script) == "function" then
        local ok, err = pcall(function() api.load_script(key) end)
        if ok then
            ok_call = true
        else
            err_call = err
        ...
        if not ok_call then
            local ok2, err2 = pcall(function() api.load_script() end)
            if ok2 then
                ok_call = true
                err_call = nil
            else
        ...
        ok_call = false
        err_call = "api.load_script missing"
    end

    if not ok_call then
        rconsolewarn("load_script: error")
        rconsolewarn(err_call)
        error(err_call)
    end

    rconsoleprint("load_script: done (stable)")
    writin("key.key", key);
end
...
if is_beta() then
	...
	save_key = function(key)
        script_key = key;
        getgenv().script_key = key;
        rconsoleprint("load_script: begin (beta)")

        local ok_call = false
        local err_call
        if type(betaapi.load_script) == "function" then
            local ok, err = pcall(function() betaapi.load_script(key) end)
            if ok then
                ok_call = true
            else
                err_call = err
            ...
            if not ok_call then
                local ok2, err2 = pcall(function() betaapi.load_script() end)
                if ok2 then
                    ok_call = true
                    err_call = nil
                else
            ...
            ok_call = false
            err_call = "betaapi.load_script missing"
        end

        if not ok_call then
            rconsolewarn("load_script: error (beta)")
            rconsolewarn(err_call)
            error(err_call)
        end

        rconsoleprint("load_script: done (beta)")
	    writin("key.key", key);
	end
...
UI["13"].Activated:Connect(function()
    ...
    if (gucci) then
        ...
        local ok_save, save_err = pcall_timeout_progress(60, function(elapsed)
            updateStatus(("Status: Loading Script... (%ds)"):format(elapsed), Color3.fromRGB(255, 255, 255))
        end, function()
            save_key(key)
        end)
        if not ok_save then
            ...
            if save_err == "timeout" then
                rconsolewarn("load_script: timeout")
